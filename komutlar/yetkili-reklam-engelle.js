...
exports.run = async (bot, message, args) => {  
  if (!args[0]) return message.channel.send(`Bu komutu kullanmak için aç yada kapat yazmalısın Örnek: ${prefix}reklam-engelle aç / kapat!`)
...
if (args[0] == 'aç') {
    var replace = await db.set(`reklamengl_${message.guild.id}`, 'acik')
      ...
      .setColor("RANDOM")
      .setDescription('Reklam Engelleme sistemi başarıyla açıldı. Artık kimse reklam yapamayacak')
  ...
  if (args[0] == 'kapat') {
    var replace = await db.set(`reklamengl_${message.guild.id}`, 'kapali')
      ...
      .setColor("RANDOM")
      .setDescription('Reklam Engelleme sistemi başarıyla kapatıldı. Sunucuda reklam yapabilecekler.')
